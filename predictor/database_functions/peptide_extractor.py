...
def extract_peptide_data(input_file_path, conditions_dictionary):
    ...
    conditions_dictionary.setdefault("Description", None)
    conditions_dictionary.setdefault("Parent Protein IRI", None)
    ...
    data = create_dictionary_uniprot_peptide_list(df_filtered)
...
def generate_df(input_file_path, conditions_dictionary):
    ...
    Gets the first sequence separated by space in peptide list.
    Modified peptides are weirdly anonated: peptide + modification
...
def apply_conditions(df, conditions_dictionary):
    ...
    Values the conditions to use:
        None:
            obligatory arguments for the generation of the df only
        Tuple:
    ...
    for condition_type, condition_values in conditions_dictionary.items():
        if condition_values is not None:
            ...
            elif condition_search == "not_contains":
                df = df[df[condition_type].str.contains(condition_value, regex=False) is False]
            elif condition_search == "match":
                df = df[df[condition_type] == condition_value]
            elif condition_search == "not_match":
                df = df[df[condition_type] != condition_value]
            elif condition_search == "is_in":
                df = df[df[condition_type].isin(condition_value)]
            else:
                raise ValueError(f"Condition search -{condition_search}- not found "
                                 f"in [contains, not_contains, match, not_match, is_in]")

    return df


def create_dictionary_uniprot_peptide_list(df):
    """
