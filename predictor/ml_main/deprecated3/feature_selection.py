...
def run_feature_importance(encoded_labeled_df):
    ...
    fig.tight_layout()
    fig.savefig("feature_importance_test_QSAR.png", bbox_inches="tight", dpi=300)
    
    selected_features = []
    for i, value in enumerate(thresholds_sorted):
        if value >= 0.005:
            selected_features.append(features_sorted[i])
...
def feature_selection():
    ...
    selected_features = run_feature_importance(encoded_labeled_df)
