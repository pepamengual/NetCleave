...
def prepare_for_LSTM(data_train, data_val, data_test, class_labels_train, class_labels_val, class_labels_test):
...
def run_LSTM(encoded_labeled_df):
    print("---> Running LSTM...")
    b_size = 128
    data_train, data_val, data_test, class_labels_train, class_labels_val, class_labels_test = splitting_data(encoded_labeled_df)
    data_train, data_val, data_test, class_labels_train, class_labels_val, class_labels_test = prepare_for_LSTM(data_train, data_val, data_test, class_labels_train, class_labels_val, class_labels_test)
    
    model = Sequential()
    model.add(Bidirectional(LSTM(data_train.shape[2], return_sequences=True, input_shape=(1, data_train.shape[2]))))
    model.add(Dropout(0.2))
    model.add(Bidirectional(LSTM(data_train.shape[2])))
    model.add(Dropout(0.2))
    model.add(Dense(1, activation='sigmoid'))
    es = EarlyStopping(monitor='val_auc', mode='max', patience=10, verbose=1)
    model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy', matthews_correlation, precision, recall, AUC()])
