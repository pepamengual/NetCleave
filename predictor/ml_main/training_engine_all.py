...
def read_table(training_data_path):
    ...
    for amino_acid in amino_acids:
        training_data_file = "{}/proteasome_{}_sequence_class.txt".format(training_data_path, amino_acid)
        table = pd.read_csv(training_data_file, sep="\t", index_col=None, header=0)
        training_table_list.append(table)
    training_table = pd.concat(training_table_list, axis=0, ignore_index=True)
...
def create_models(training_data_path, models_export_path):
    ...
    model.save_weights("{}/proteasome_all_models.h5".format(models_export_path))
