...
def export_df_for_ml(large_uniprot_peptide, random_peptides, amino_acid_list, frequency_random_model_list, n, proteasome_ml_path, erap_ml_path):
...
def get_cleavage_region(large_uniprot_peptide, random_peptides, n):
    ...
    erap_cleavage_regions_ms = [large_uniprot_peptide[i][2:12] for i in range(len(large_uniprot_peptide))]
    erap_cleavage_regions_random_1 = [large_uniprot_peptide[i][3:13] for i in range(len(large_uniprot_peptide))]
    erap_cleavage_regions_random_2 = [large_uniprot_peptide[i][4:14] for i in range(len(large_uniprot_peptide))]
    erap_cleavage_regions_random_3 = [large_uniprot_peptide[i][1:11] for i in range(len(large_uniprot_peptide))]
    erap_cleavage_regions_random_4 = [large_uniprot_peptide[i][0:10] for i in range(len(large_uniprot_peptide))]
    
    erap_cleavage_regions_random_all = erap_cleavage_regions_random_1 + erap_cleavage_regions_random_2 + erap_cleavage_regions_random_3 + erap_cleavage_regions_random_4
    erap_cleavage_regions_random = list(set(erap_cleavage_regions_random_all) - set(erap_cleavage_regions_ms))
